...
impl TabState {
    pub fn new(tab: &str) -> Self {
        ...
        let tree = egui_dock::DockState::new(vec![tab.to_string()]);
        let tab_index = 0;
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        added_nodes.drain(..).for_each(|tab_context| {
            ...
            self.tree.push_to_focused_leaf(self.tab_index.to_string());
            self.tab_index += 1;
...
impl Default for TabState {
    fn default() -> Self {
        let tree = egui_dock::DockState::new(vec![String::new()]);
        let tab_index = 0;
        let notify = egui_notify::Toasts::default();
        Self {
            tree,
